...
class Project extends AppModel {
	  ...
	  function getLatestProjects($filters, $latest =1)
	  {
	 	 	...
	 	 	"fields"=>"Project.*, User.profilepic, (SELECT SUM(weight) FROM project_tasks WHERE project_tasks.project_id = Project.id) as completed, (SELECT COUNT(id) FROM project_student_task_docs as pstdo WHERE pstdo.project_id = Project.id) as noOfFiles, (SELECT COUNT(id) FROM project_comments  as psc WHERE psc.project_id = Project.id) as noOfComments, Subject.title",
