		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
				...
				if(!in_array(strtolower($this->ext($fileName)),$this->_allowed)){
					$this->error("Please browse valid images file. Only .jpg, .gif, .png file format are allowed!");
				} else {
						...
						if ($rules == NULL) {
							...
							if (function_exists("imagecreatefromjpeg")) {
								...
								if (isset($rules['type']) && isset($rules['size'])) {
									...
									$ImageName = $this->image($this->_file, $rules['type'], $rules['size'], $rules['output'], $rules['quality']);
...
//			$uploadWidth  = $uploadSize[2];
//			$uploadHeight = $uploadSize[3];
//
//			$uploadWidth  = $uploadSize[4];
//			$uploadHeight = $uploadSize[5];
//
//			$uploadWidth  = $uploadSize[6];
//			$uploadHeight = $uploadSize[7];
//
//			$uploadWidth  = $uploadSize[8];
//			$uploadHeight = $uploadSize[9];
//
//			$uploadWidth  = $uploadSize[10];
//			$uploadHeight = $uploadSize[11];
