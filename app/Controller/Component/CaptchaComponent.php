...
class CaptchaComponent extends Component
{
	...
	*/
	...
	function create()	{
		...
		$credits = 'PHP-ORACLE'; # if empty, HTTP_HOST will be shown
		...
		while(true){
			...
			// LOGIC TO DRAW LINES BEHIND THE GENERATED CAPTCHA IMAGE
			
			 for ($i = 0; $i < 30; $i++) {
				// allocate colour
				
				$iRandColour = rand(100, 250);
				$iLineColour = imagecolorallocate($img, $iRandColour, $iRandColour, $iRandColour);
				// draw line
				imageline($img, rand(0, $width), rand(0, $height), rand(0, $width), rand(0, $height), $iLineColour);
			 }
...
// 			header("Content-Type: image/x-png");
		...
		if(isset($oldcaptcha))	{
			if(file_exists(FILES_PATH.'captcha/'.$oldcaptcha)) {		
