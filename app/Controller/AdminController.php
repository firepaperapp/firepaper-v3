...
//			$this->breadcrumb['Home']='/admin/home/';
	...
	function addedituser($user_id=0) {
		...
		if(!isNull($this->request->data)) {
			...
			$err = $this->User->validateOnAdminEnd($this->request->params['data']['User'], $this->request->params['form']['uid']);
...
/******************************Send email after creating the user *************/
	...
	function changepassword() {
		...
		if(!isNull($this->request->data)) {
			...
			$err = $this->Admin->validateAllPasswords($this->request->data['Admin'], $_SESSION["aid"]);
